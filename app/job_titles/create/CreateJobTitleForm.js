...
function SaveButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-medium transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {pending ? "Saving..." : "Save"}
    </button>
  );
...
function SaveAndCreateAnotherButton({ onClick }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={pending}
      className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg font-medium transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
    >
...
export default function CreateJobTitleForm({ jobPositions, onSubmit }) {
  ...
  const handleSaveAndCreateAnother = async () => {
    ...
    const data = {
      ...
      job_position_id: formDataObj.get("job_position_id") ? parseInt(formDataObj.get("job_position_id")) : null,
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link href="/job_titles" className="hover:text-gray-700 cursor-pointer">
          Job Titles
        </Link>
      ...
      <form id="createJobTitleForm" onSubmit={handleSubmit}>
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6 mb-8">
            ...
            {/* Job Position */}
            <div>
              <label htmlFor="job_position_id" className="block text-sm font-medium text-gray-700 mb-2">
                Job Position
              </label>
              <select
                id="job_position_id"
