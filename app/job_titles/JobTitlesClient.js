...
import JobTitleCard from "@/app/components/JobTitleCard";
import JobTitleListItem from "@/app/components/JobTitleListItem";
...
export default function JobTitlesClient({ 
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      router.push(`/job_titles?${params.toString()}`);
    }, 500); // 500ms debounce
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/job_titles/${deleteModal.id}`, {
        method: 'DELETE',
      });
  ...
  const toggleAll = () => {
    if (selectedJobTitles.length === jobTitles.length) {
      setSelectedJobTitles([]);
    } else {
      setSelectedJobTitles(jobTitles.map((d) => d.id));
    }
  };

  // Filter jobTitles (client-side filters only, search is handled by API)
  const filteredJobTitles = jobTitles.filter((jp) => {
  ...
  // Sort jobTitles
  const sortedJobTitles = [...filteredJobTitles].sort((a, b) => {
    if (sortBy === "name-asc") {
  ...
  // Group jobTitles
  const groupedJobTitles = React.useMemo(() => {
    if (groupBy === "-") {
      return { "All Job Titles": sortedJobTitles };
    } else if (groupBy === "job_position_id") {
      return sortedJobTitles.reduce((groups, jp) => {
        const key = jp.job_position_id || "No Level";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
      ...
      {/* Selection Row + Job Title Grid/List */}
      <div className="p-6">
        {viewMode === "grid" && groupBy === "-" && (
          <div className="flex items-center gap-3 mb-5">
            <input
              ...
              checked={
                selectedJobTitles.length === sortedJobTitles.length &&
        ...
        {viewMode === "grid" && (
          <div>
            {Object.entries(groupedJobTitles).map(([groupName, jobTitlesGroup]) => (
              <div key={groupName} className="mb-6">
                {groupBy !== "-" && (
                  <h3 className="text-sm font-semibold text-gray-700 mb-4 pb-2 border-b border-gray-200">
                    {groupName} ({jobTitlesGroup.length})
                  </h3>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {jobTitlesGroup.map((jobTitle) => (
                    <JobTitleCard
                      key={jobTitle.id}
                      jobTitle={jobTitle}
                      isSelected={selectedJobTitles.includes(jobTitle.id)}
                      ...
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="p-2 text-left w-12">
                          <input
                            ...
                            checked={
                              jobTitlesGroup.every(jp => selectedJobTitles.includes(jp.id)) &&
                              jobTitlesGroup.length > 0
                            }
                            onChange={() => {
                              const allSelected = jobTitlesGroup.every(jp => selectedJobTitles.includes(jp.id));
                              if (allSelected) {
                                setSelectedJobTitles(prev => 
                                  prev.filter(id => !jobTitlesGroup.find(jp => jp.id === id))
                                );
                              } else {
                                setSelectedJobTitles(prev => [
                                  ...prev,
                                  ...jobTitlesGroup.filter(jp => !prev.includes(jp.id)).map(jp => jp.id)
                                ]);
                              }
                            }}
                          />
