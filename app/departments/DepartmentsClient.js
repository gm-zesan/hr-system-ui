...
export default function DepartmentsClient({
    ...
    const handleDeleteConfirm = async () => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/departments/${deleteModal.id}`,
                {
                    method: "DELETE",
                }
            );
    ...
    const filteredDepartments = departments.filter((dept) => {
        ...
        const matchesManager = !filterManager || dept.manager_name === filterManager;
                ...
                {sortedDepartments.length > 0 && (
                    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-lg border border-gray-200">
                        ...
                        <div className="text-sm text-gray-600">
                            ...
                            of <span className="font-medium">{total}</span> departments
                        </div>
