...
export default function JobPositionDetailsLayout({ children, params }) {
  ...
  const isViewActive = pathname === `/job_positions/${jobPositionId}`;
  const isEditActive = pathname === `/job_positions/${jobPositionId}/edit`;
    ...
    <>
      ...
      <div className="mb-6 flex justify-center">
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm inline-flex p-2">
          <TabLink
            href={`/job_positions/${jobPositionId}`}
            label="View"
            icon={Eye}
            isActive={isViewActive}
          />
          <TabLink
            href={`/job_positions/${jobPositionId}/edit`}
            label="Edit"
            icon={Edit}
