...
export default function SkillLayout({ children }) {
    ...
    const isViewActive = pathname === `/configurations/skills/${id}`;
    const isEditActive = pathname === `/configurations/skills/${id}/edit`;
        ...
        <>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <h1 className="text-3xl font-bold text-gray-900">
                    {isViewActive && "View Skill"}
                    {isEditActive && "Edit Skill"}
                </h1>
            </div>
            ...
            <div className="mb-6 flex justify-center">
                <div className="bg-white border border-gray-200 rounded-lg shadow-sm inline-flex p-2">
                    <TabLink
                        href={`/configurations/skills/${id}`}
                        label="View"
                        isActive={isViewActive}
                    />
                    <TabLink
                        href={`/configurations/skills/${id}/edit`}
                        label="Edit"
