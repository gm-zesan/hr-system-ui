...
export default function JobTitlesClient({ 
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      router.push(`/configurations/job_titles?${params.toString()}`);
    }, 500); // 500ms debounce
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/configurations/job_titles/${deleteModal.id}`, {
        method: 'DELETE',
      });
