...
export default function JobPositionsClient({ 
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      router.push(`/configurations/job_positions?${params.toString()}`);
    }, 500); // 500ms debounce
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/configurations/job_positions/${deleteModal.id}`, {
        method: 'DELETE',
      });
