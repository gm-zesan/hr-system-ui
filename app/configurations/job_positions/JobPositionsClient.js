...
export default function JobPositionsClient({
    ...
    const handleDeleteConfirm = async () => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/configurations/job_positions/${deleteModal.id}`,
                {
                    method: "DELETE",
                }
            );
    ...
    const filteredJobPositions = jobPositions.filter((jp) => {
        ...
        const matchesCode =
            !filterCode || (jp.code && jp.code.toLowerCase().includes(filterCode.toLowerCase()));
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
                                    ...
                                    <tbody>
                                        {jobPositionsGroup.map((jobPosition) => (
                                            <JobPositionListItem
                                                ...
                                                isSelected={selectedJobPositions.includes(
                                                    jobPosition.id
                                                )}
                ...
                {sortedJobPositions.length > 0 && (
                    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-lg border border-gray-200">
                        ...
                        <div className="text-sm text-gray-600">
                            ...
                            of <span className="font-medium">{total}</span> job positions
                        </div>
            ...
            {deleteModal.show && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    {/* Modal */}
                    <div
                        className={`relative bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6 transition-all duration-300 ${
