...
export default function TagsClient({
    ...
    useEffect(() => {
        ...
        searchTimeoutRef.current = setTimeout(() => {
            ...
            params.set("page", "1");
            router.push(`/configurations/tags?${params.toString()}`);
        }, 500);
    ...
    const handleDeleteConfirm = async () => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/configurations/tags/${deleteModal.id}`,
                {
                    method: "DELETE"
                }
            );
    ...
    const toggleAll = () => {
        if (selectedTags.length === tags.length) {
            ...
            setSelectedTags(tags.map((d) => d.id));
        }
    };

    // Filter tags (client-side filters only, search is handled by API)
    const filteredTags = tags.filter((tag) => {
        ...
        const matchesCode =
            !filterCode || (tag.code && tag.code.toLowerCase().includes(filterCode.toLowerCase()));
    ...
    const groupedTags = React.useMemo(() => {
        if (groupBy === "-") {
            ...
            return sortedTags.reduce((groups, tag) => {
                const key = tag.is_active ? "Active" : "Inactive";
