...
export default function ManageResumesPage({ params }) {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      if (displayTypeDropdownRef.current && !displayTypeDropdownRef.current.contains(event.target)) {
        ...
        <div className={`fixed inset-0 backdrop-blur-sm bg-black/30 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ${isAnimating && !isClosing ? 'opacity-100' : 'opacity-0'}`}>
          <div className={`bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto transition-all duration-300 ${isAnimating && !isClosing ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">Create Employee Resume</h2>
              <button
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="relative" ref={typeDropdownRef}>
                    ...
                    >
                      <span className={`text-sm ${formData.type ? 'text-gray-900' : 'text-gray-400'}`}>
              ...
              <div className="mt-6">
                ...
                <div className="relative" ref={displayTypeDropdownRef}>
                  <button
                    onClick={() => setShowDisplayTypeDropdown(!showDisplayTypeDropdown)}
                  ...
                  >
                    <span className={`text-sm ${formData.displayType ? 'text-gray-900' : 'text-gray-400'}`}>
                      {formData.displayType || 'Select an option'}
                    </span>
                    <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform ${showDisplayTypeDropdown ? 'rotate-180' : ''}`} />
                  </button>
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto">
                      {displayTypeOptions.map((option) => (
                        <button
                          key={option}
                          onClick={() => {
                            setFormData({ ...formData, displayType: option });
                            setShowDisplayTypeDropdown(false);
                          }}
                          className="w-full px-4 py-2.5 text-left text-sm text-gray-900 hover:bg-gray-50 transition-colors"
                        >
